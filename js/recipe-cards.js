(function () {
  function flattenRecipes(data) {
    ...
    function walkAltKat(altKat, kategoriAd) {
      (altKat.tarifler || []).forEach(t => {
        list.push({ ...t, _kategori: kategoriAd, _altKategori: altKat.ad });
      });
    }

    (data.kategoriler || []).forEach(kat => {
      (kat.alt_kategoriler || []).forEach(altKat => walkAltKat(altKat, kat.ad));
    });
  ...
  async function loadRecipes() {
    try {
      const res = await fetch('./Yemekler5.json');
      if (!res.ok) throw new Error('JSON yüklenemedi');
      const data = await res.json();
      return flattenRecipes(data);
    } catch (e) {
      console.error('Tarif yüklenirken hata:', e);
      return [];
    }
  }
  function createCard(recipe, index) {
    ...
    const stepCount  = (recipe.adimlar  || []).length;
    ...
    card.innerHTML = `
      <div class="rc-inner">
        ...
        ${breadcrumb ? `<p class="rc-breadcrumb">${breadcrumb}</p>` : ''}
  ...
  function injectGrid(recipes) {
    let section = document.querySelector('.recipe-grid-section');
    if (!section) {
    ...
    function render(list) {
      ...
      list.forEach((r, i) => {
        ...
        c.style.animationDelay = `${Math.min(i, 14) * 50}ms`;
        grid.appendChild(c);
      });
    }
    ...
    srch.addEventListener('input', function () {
      ...
      render(recipes.filter(r =>
        ...
        (r._altKategori || '').toLowerCase().includes(q) ||
        (r.malzemeler || []).some(m => (m.ad || '').toLowerCase().includes(q))
      ));
    });
  }
  ...
  function openModalFull(recipe) {
    ...
    const ul = modalEl.querySelector('.rm-ingredients-list');
    ul.innerHTML = (recipe.malzemeler || []).map(m => `
      <li class="rm-ingredient">
  ...
  function syncStep() {
    ...
    const txt  = modalEl.querySelector('.rm-step-text');
  ...
  function syncDots() {
    const el = modalEl.querySelector('.rm-dots');
    el.innerHTML = '';
    steps.forEach((_, i) => {
      const b = document.createElement('button');
      b.className = 'rm-dot-btn' + (i === currentStep ? ' active' : '');
      b.setAttribute('aria-label', `Adım ${i + 1}`);
      b.addEventListener('click', () => { currentStep = i; syncStep(); syncNav(); syncDots(); });
      el.appendChild(b);
    });
  }
  ...
  function buildModal() {
    ...
    modalEl.querySelector('.rm-prev').addEventListener('click', () => go(-1));
    modalEl.querySelector('.rm-next').addEventListener('click', () => go(1));
    ...
    let tx = 0;
    modalEl.addEventListener('touchstart', e => { tx = e.changedTouches[0].clientX; }, { passive: true });
...
.rc-breadcrumb{font-family:'Poppins',sans-serif;font-size:10px;color:rgba(255,255,255,.28);margin:0;letter-spacing:.03em}
...
.rm-dot-btn:hover:not(.active){background:rgba(255,255,255,.28)}
  ...
  async function init() {
    ...
    if (recipes.length) injectGrid(recipes);
    else console.warn('Yemekler5.json boş veya yüklenemedi.');
  }
